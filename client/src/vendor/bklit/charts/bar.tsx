...
export interface BarProps {
  ...
  minBarHeight?: number;
  /**
   * Bayt addition (upstream bar.tsx has no per-item color): resolves the
   * fill for one bar from its row and index, falling back to `fill` when it
   * returns nothing. `fill` alone is one color for the whole series — this
   * is for a series whose sign changes what a bar means, e.g. months kept
   * versus months overspent, where a single flat color would say the same
   * thing about both.
   */
  fillFor?: (row: Record<string, unknown>, index: number) => string | undefined;
}
  ...
  return (
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        const itemFill = fillFor?.(d, i) ?? fill;
        ...
        if (isHorizontal) {
          ...
          if (stacked && stackOffsets) {
            ...
            // Bayt addition: upstream measured every bar from the chart's own
            // pixel floor (innerHeight) as if that were always the zero line,
            // which is only true when every value is non-negative — the shape
            // every bklit example ships in. A y-scale whose domain spans
            // negative to positive puts zero somewhere in the middle
            // (scale(0)), and a bar below it has valuePos > innerHeight is
            // never true — it just makes `innerHeight - valuePos` collapse
            // toward zero and go negative, which is the invalid <rect height>
            // this chart was throwing. Measuring from the scale's own zero
            // instead grows a bar up for a positive value and down for a
            // negative one, same as it always did wherever zero is the floor.
            const baselineY = scale(0) ?? innerHeight;
            barHeight = value >= 0 ? baselineY - valuePos : valuePos - baselineY;
            y = value >= 0 ? valuePos : baselineY;
