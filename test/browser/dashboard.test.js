...
async function pickMonth(page, delta) {
  ...
  const current = await page
    .locator('.month-trigger .short, .month-trigger .long')
    .first()
    .innerText();
...
(async () => {
  ...
  await page.locator('input.person-name').nth(1).fill('Wife');
  ...
  await go('Activity');
  check(
    'an entry can be recorded from the strip',
    (await page.locator('.txn:not(.empty)').count()) === 1,
  );
  await go('Home');
  ...
  check(
    'filtering to spending hides the salary',
    (await page.locator('.txn:not(.empty)').count()) === 0,
  );
  ...
  check(
    'and it refreshed behind them anyway',
    (await page.locator('.txn-table .txn').count()) === seen,
  );
  ...
  check(
    'clicking one opens the same editor Activity uses',
  ...
  check(
    ...
    wasFirst !== (await page.locator('.latest .txn').first().textContent()).trim(),
  );
  ...
  check(
    'the page cannot scroll behind an open dialog',
    (await page.evaluate(() => window.scrollY)) === held,
  );
  ...
  check(
    'and it is preset to that account',
    Boolean(await page.locator('.sheet select').inputValue()),
  );
  ...
  await go('Activity');
  check(
    'the entry it made is in the list',
    (await page.locator('.txn', { hasText: 'Fuel' }).count()) === 1,
  );
  ...
  check(
    'and each person has an initial',
    (await page.locator('.person-head .avatar').count()) >= 1,
  );
  ...
  check(
    'and only the person who holds both gets one',
  ...
  check(
    'the month card counts the subscription',
    flowAfter.includes('1 item'),
    flowAfter.slice(0, 200),
  );
  ...
  // The donut moved from hand-drawn <circle> strokes to a real bklit
  // PieChart, which draws each slice as two <path> elements — the visible
  // arc, and a transparent one sized to the full slice that carries the
  // hover events so a thin ring is still easy to point at. Only the first
  // is a slice as far as this check is concerned.
  const slices = await page.locator('.donut path:not([fill="transparent"])').count();
  const legend = await page.locator('.donut-legend li').count();
  check(
    'where it went is a donut with a legend per slice',
    slices > 0 && slices === legend,
  ...
  await pickMonth(page, -1);
  check(
    'last month has none of this month’s entries',
    (await page.locator('.txn:not(.empty)').count()) === 0,
  );
  await backToThisMonth(page);
  ...
  check(
    'with a word on why',
    (await page.locator('.modal').textContent()).includes(
      'Every row needs an account and an amount',
    ),
  );
  await page.locator('.split-row').nth(1).locator('.split-amount').fill('2000');
  await page.waitForTimeout(300);
  check(
    'rows can be added for more than one destination',
    (await page.locator('.split-row').count()) === 2,
  );
  check(
  ...
  await page.waitForTimeout(400);
  check(
    'going over the balance is flagged',
    (await page.locator('.split-total.over').count()) === 1,
  );
  check(
  ...
  await page.waitForTimeout(400);
  check(
    'reducing it enables the button again',
    !(await page.locator('.modal button[type="submit"]').isDisabled()),
  );
  ...
  check(
    'shares are measured against the balance by default',
    (await page.locator('.basis button.active').textContent()).includes('This balance'),
  );
  ...
  check(
    ...
    (await page.locator('.basis-figure').textContent()).includes(
      'recurring income paid into this account',
  ...
  const savingsTile = () =>
    page
      .locator('.breakdown .k')
      .filter({ hasText: /Moved|From savings/ })
      .first();
  ...
  check('and shows it as a positive figure', (await figure()).includes('2,000'), await figure());
