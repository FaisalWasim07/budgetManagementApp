...
async function run() {
  ...
  //
  // The endpoint is the address the push service gave us and is unique on its
  // own, so re-subscribing the same browser updates rather than duplicating.
  // The two keys are what the payload is encrypted to; the server can write
  // them but never reads anything back through them.
  await db.exec(`
    CREATE TABLE IF NOT EXISTS push_subscriptions (
      ...
      auth        text NOT NULL,
      -- Only to tell one device from another in a list. Never matched on.
      label       text,
      created_at  timestamptz NOT NULL DEFAULT now(),
      last_sent_at timestamptz,
      -- Set when the push service says the endpoint is gone, so a dead device
